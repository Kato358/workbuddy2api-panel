...
预构建镜像由 GitHub Actions 推到 GHCR。两个独立 workflow：

1. **Sync upstream** — 每天 04:17 UTC（也可手动）merge [`linguo2625469/workbuddy2api-panel`](https://github.com/linguo2625469/workbuddy2api-panel) 的 `main`。有新提交才 push；冲突则失败，需手工处理。
2. **Docker image** — 仅手动 / 由同步成功后触发。构建 `linux/amd64` + `linux/arm64`，打 `latest` 与 `sha-<short>`。
...
首次推送后到 GitHub Packages 把 `workbuddy2api-panel` 设为 Public，否则未登录拉不到。Fork 需先在仓库 Settings → Actions 打开 workflow。
